...
function magnews_orderby_post(){
    return array(
        'none' => __('No order', 'ahopeelement'),
        'ID' => __('Post ID', 'ahopeelement'),
        'author' => __('Author', 'ahopeelement'),
        'title' => __('Title', 'ahopeelement'),
        'date' => __('Published date', 'ahopeelement'),
        'modified' => __('Modified date', 'ahopeelement'),
        'parent' => __('By parent', 'ahopeelement'),
        'rand' => __('Random order', 'ahopeelement'),
        'comment_count' => __('Comment count', 'ahopeelement'),
        'menu_order' => __('Menu order', 'ahopeelement'),
        'post__in' => __('By include order', 'ahopeelement'),
    );  
}
...
*/
 if ( ! function_exists( 'ahope_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function ahope_posted_on() {
...
function madmag_ft_images($thumbnail){
    ...
    $placeholder =  Ahope_PLUGIN_URL . 'assets/lazyload.svg';
...
function ahope_clearifix(){
    $out='<div class="meta-clearing"></div>';
    return $out;
}

function ahope_clearifix_h5(){
    $out='<div class="meta-clearing height5"></div>';
    return $out;
}

function ahope_clearifix_h10(){
...
if ( ! function_exists( 'ahope_single_category_bg' ) ) :

        function ahope_single_category_bg($default = true) {
...
if ( ! function_exists( 'ahope_single_category' ) ) :

        function ahope_single_category() {
...
function ahope_build_postmeta($metas = '',$excerpt_length = '',$readmore='',$firsthumb=''){
        ...
        if ( is_array($metas) ) {
            ...
            foreach( $metas as $meta ) {
                    ...
                    case 'cat':
                        $out = ahope_single_category(false);
                        break;

                    case 'cat_bg':
                        $out = ahope_single_category_bg(false);
                        break;
                    ...
                    case 'h5':
                        $out = ahope_clearifix_h5() ;
                        break;

                    case 'h10':
                        $out = ahope_clearifix_h10() ;
                        break;


                    case 'btn':
                        $out = ahope_readmore_btn($readmore) ;
                        break;
...
function ahope_single_category($default = true) {
    if ( 'post' == get_post_type() ) {
        $categories = get_the_category();
        $separator = ',';
        $output = '';
...
function ahope_get_category_link($taxonomy){
...
function ahope_drop_posts($post_type){
    $args = array(
      'numberposts' => -1,
      'post_type'   => $post_type
    );
...
function misha_loadmore_ajax_handler(){
    ...
    require Ahope_PLUGIN_DIR.'includes/loops/'. $template .'.php';
    die(); 
}
...
function misha_tabcat_ajax_handler(){
    ...
    //require(Ahope_PLUGIN_DIR.'includes/loops/tabcat.php');
    require Ahope_PLUGIN_DIR.'includes/loops/'. $template .'.php';
    die(); 
}
