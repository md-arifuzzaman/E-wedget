 ...
 * @return array
 */
function appal_body_classes($classes)
{
...
add_filter('body_class', 'appal_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function appal_pingback_header()
{
...
add_action('wp_head', 'appal_pingback_header');

if (!function_exists('appal_readmore_content')) :
    /**
     * Prints readmore content
     */
    function appal_readmore_content($word)
    {
        echo wp_trim_words(get_the_content(), $word, '');
    }
endif;
function appal_theme_meta($meta){
    return get_post_meta(get_the_ID(), "_appal_$meta", true);
}
function appal_theme_options($option){
    global $appal;
    if (isset($appal[$option])) {
        return $appal[$option];
    }
}

function appal_dynamic_header()
{
    $header_switch = appal_theme_meta('meta_header_switch');
    $opt_header = appal_theme_options('appal_home_header');
    $opt_page_header = appal_theme_options('appal_page_header');

    if ($header_switch == 'header_on') {
        do_action('appal_header');
    } else {
        if (!is_front_page() && !empty($opt_header)) {
            echo do_shortcode('[INSERT_ELEMENTOR id="' . $opt_page_header . '"]');
        } elseif (is_front_page() && !empty($opt_header)) {
...
function appal_dynamic_footer()
{
    $footer_switch = appal_theme_meta('meta_footer_switch');
    $opt_footer = appal_theme_options('appal_global_footer');

    if ($footer_switch == 'footer_on') {
        do_action('appal_footer');
    } else {
        if (!empty($opt_footer)) {
...
function appal_single_title($arg){

    if ( is_category() ) {
        /* translators: Category archive title. 1: Category name */
        $title = single_cat_title( $arg['cat'], 'appal');
    } elseif ( is_tag() ) {
        /* translators: Tag archive title. 1: Tag name */
        $title = single_cat_title( $arg['tag'], 'appal');
    } elseif ( is_author() ) {
        ...
        $title = esc_html__( 'Home', 'appal' );
    } elseif( is_singular() ){
        $title = get_the_title();
    }else {
        $title = esc_html__( 'Archives','appal' );
    }

    return $title;
}
function appal_unit_breadcumb($separator = '') {
    /* === OPTIONS === */
    ...
    $link          .= '<a class="breadcrumbs-link" href="%1$s" itemprop="item"><li class="breadcrumb-item" itemprop="name">%2$s</li></a>';
